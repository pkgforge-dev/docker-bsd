...
nothing in this repository's gate depends on it. It is a convenience for a
person, not a dependency of a measurement.
...
- ⛔ **From a script, pass `-CommandB64` rather than `-Command`.** Windows
  ...
  list, so a 5.1 caller that passes `-Command` loses characters.
- ⚠ `-TimeoutSeconds` bounds the script's own questions to the distribution. It
  does **not** bound `-Command`.
