...
| ⛔ **Editing a shell script while a shell is executing it** | ⛔ **the interpreter resumes at a BYTE OFFSET, not at a line.** Inserting a line ahead of the read position shifts everything after it, and the shell resumes mid-statement. On 2026-08-28 a one-line `# shellcheck disable` comment was added to `experiments/47-comp-set-and-compile.sh` forty minutes into its run; the container's own work finished and printed its result correctly, then the wrapper died with `line 270: -c: command not found` and exit 127. ⚠ **The measurement survived and the exit code lied about it**, which is the expensive half. ⭐ Wait, or copy the script and edit the copy. |
| ⛔ **A done-condition that matches the command instead of its output** | ⛔ **the log carries both.** A hold loop polling for `BENCH rc=` matched the echoed command line `echo "BENCH rc=$R ..."` and reported the run complete on its first tick, over a compile with fifty minutes left to go. ⚠ Same class as matching a tty's echo, in a different place, and this repository has now paid for it twice. ⭐ Match on something only the OUTPUT can contain: `BENCH rc=[0-9]` cannot match `rc=$R`. |
| ⛔ **A hold loop whose "cheap check" touches what it is waiting on** | ⛔ **the poll becomes the contention.** A tick calling `podman machine ssh` every 45 s goes through the same `wsl.exe` a running gate was using; that gate then sat in one check for over sixteen minutes and had to be killed, and the same check answered in seconds on its own straight afterwards. ⚠ Not proved to be the cause, and the only unusual thing about the run. ⭐ Tick on a FILE the work already writes, and never on a service it already holds. |
